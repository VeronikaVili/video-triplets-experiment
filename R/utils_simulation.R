...
complete_responses <- function(df) {
  ...
  d_complete <- tibble(expand.grid(orig = stimuli_id, left = stimuli_id, right = stimuli_id,stringsAsFactors = F), response = NA_integer_,selected = NA_integer_, rt = NA_real_)
